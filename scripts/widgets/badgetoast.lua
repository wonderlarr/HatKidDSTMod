...
local BadgeToast = Class(Widget, function(self, owner, controls)
    Widget._ctor(self, "BadgeToast")
    self.controls = controls
    self.owner = owner
    self.root = self:AddChild(Widget("ROOT"))

    self.tab_gift = self.root:AddChild(UIAnimButton("tab_badgeseller", "tab_badgeseller", nil, nil, "off", nil, nil))
    self.tab_gift:Disable()

    self.tab_gift:SetLoop("active_loop", true)

    self.tab_gift:SetTooltip("Badge Seller")
    self.tab_gift:SetTooltipPos(0, -40, 0)

    self.inst:ListenForEvent("badgeseller", function(player, data)
       self:UpdateElements()
    end, ThePlayer)
...
function BadgeToast:EnableClick()
...
function BadgeToast:OnClickEnabled()
...
function BadgeToast:DisableClick()
    self.tab_gift:Disable()
...
function BadgeToast:UpdateElements()
    local from = self.root:GetPosition()

    if not self.controller_hide and not self.craft_hide and self.owner.player_classified and true then 
...
function BadgeToast:ToggleHUDFocus(focus)
    self.hud_focus = focus
    self:UpdateControllerHelp()
end

function BadgeToast:ToggleController(hide)
    self.controller_hide = hide
    self:UpdateElements()
end

function BadgeToast:ToggleCrafting(hide)
    self.craft_hide = hide
    self:UpdateElements()
end

--Called from PlayerHud:OnControl
function BadgeToast:CheckControl(control, down)
    if self.shown and down and control == CONTROL_INSPECT_SELF then
        return true
    end
end

function BadgeToast:UpdateControllerHelp()
